...
import {
  ...
  TooManyTypeNames,
} from "quicktype-core/dist/attributes/TypeNames";
import { transformationForType } from "quicktype-core/dist/Transformers";
import { TypeGraph } from "quicktype-core/dist/TypeGraph";
import { matchCompoundType } from "quicktype-core/dist/TypeUtils";
...
export function gatherNames(
  ...
  while (!queue.isEmpty) {
    ...
    if (t instanceof ObjectType) {
      ...
      if (values !== undefined) {
        addNames(values, names === null ? null : setMap(names, singular));
      }
    } else {
      matchCompoundType(
        t,
        (arrayType) => {
          addNames(
            arrayType.items,
            names === null ? null : setMap(names, singular)
          );
        },
  ...
  if (debugPrint) {
    for (const t of graph.allTypesUnordered()) {
      ...
      console.log(
        `${index}: ${
          names === null ? "*** too many ***" : Array.from(names).join(" ")
        }`
      );
    }
  }
      ...
      if (ancestor !== undefined && ancestorAlternatives !== null) {
        ...
        if (ancestorNames === null) {
          ...
          for (const name of names) {
            alternatives.push(
              ...Array.from(ancestorNames).map((an) => `${an}_${name}`)
            );
      ...
      matchCompoundType(
        ...
        (unionType) => {
          ...
          const unionIsAncestor =
            unionHasGivenName || nullableFromUnion(unionType) === null;
