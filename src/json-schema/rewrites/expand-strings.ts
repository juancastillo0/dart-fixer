...
import { stringTypesForType } from "quicktype-core/dist/TypeUtils";
...
export function expandStrings(
  ...
  const allStrings = Array.from(graph.allTypesUnordered()).filter(
    (t) =>
      t.kind === "string" && stringTypesForType(t as PrimitiveType).isRestricted
  ) as PrimitiveType[];
  ...
  function replaceString(
    ...
    types.push(
      ...Array.from(transformations).map((k) => builder.getPrimitiveType(k))
    );
