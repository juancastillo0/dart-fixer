...
import { MarkovChain, load, evaluate } from "quicktype-core/dist/MarkovChain";
import {
  isPrimitiveStringTypeKind,
  setOperationCasesEqual,
} from "quicktype-core/dist/Type";
import { TypeGraph } from "quicktype-core/dist/TypeGraph";
import { removeNullFromType } from "quicktype-core/dist/TypeUtils";
import { unifyTypes, unionBuilderForUnification } from "../core/unify-classes";
...
function shouldBeMap(
  ...
  if (
    ...
    iterableEvery(
      properties.values(),
      (cp) => isPrimitiveStringTypeKind(cp.type.kind) || cp.type.kind === "null"
    )
  ) {
    return undefined;
  }
  ...
  for (const [, p] of properties) {
    ...
    if (nn.size > 0) {
      if (firstNonNullCases !== undefined) {
        ...
        if (
          !setOperationCasesEqual(nn, firstNonNullCases, true, (a, b) =>
