...
} from "quicktype-core/dist/attributes/TypeAttributes";
import {
  TypeLookerUp,
  GraphRewriteBuilder,
} from "quicktype-core/dist/GraphRewriting";
import { mustNotHappen } from "quicktype-core/dist/support/Support";
import {
  IntersectionType,
    ...
    >
{
  ...
  private updateArrayItemTypes(members: Iterable<Type>): void {
    const maybeArray = iterableFind(members, (t) => t instanceof ArrayType) as
      | ArrayType
      | undefined;
    ...
    matchTypeExhaustive<void>(
      ...
      (unionType) => {
        attributes = combineTypeAttributes(
          "intersect",
          [attributes].concat(
            Array.from(unionType.members).map((m) => m.getAttributes())
          )
        );
